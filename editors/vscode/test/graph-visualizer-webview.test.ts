...
test('keeps the browser client bounded and treats snapshot text as inert', async () => {
  ...
  assert.match(source, /if \(drawn >= MAX_LABELS\) break;/u);
  assert.match(source, /const priority = node\.id === state\.selectedId \|\| node\.id === state\.keyboardId;[\s\S]*?occupied\.some\(\(entry\) => rectanglesOverlap\(entry, bounds\)\)/u);
  ...
  assert.match(source, /if \(edge\.source === edge\.target\) drawSelfLoop\(source\);/u);
  ...
  assert.match(source, /function drawSelfLoop\(point: Point\): void \{[\s\S]*?context\.arc\([\s\S]*?context\.fill\(\);/u);
  ...
  assert.match(source, /const cellRadius = Math\.max\(1, Math\.min\(2, Math\.ceil\(25 \/ state\.scale \/ POSITION_CELL\)\)\);/u);
