---
name: graphoxide
description: Query and maintain a deterministic code knowledge graph from Copilot.
---

# Graphoxide

Use `graphoxide query`, `graphoxide explain`, and `graphoxide path` before broad source searches.
Rebuild with `graphoxide update .` after structural changes. See `references/` for focused workflows.
