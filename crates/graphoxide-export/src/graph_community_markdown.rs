//! Portable graph-community Markdown export.

use crate::{
    graph_community_topics::cross_community_relationships,
...
/// One Markdown file in a graph-community Markdown render plan.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct GraphCommunityMarkdownPage {
    /// A normalized path relative to the graph-community Markdown root.
    pub path: String,
    /// Complete Markdown, including frontmatter.
    pub markdown: String,
}

/// Deterministic, pure Markdown pages for the graph-community hierarchy.
#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct GraphCommunityMarkdownPlan {
    pub pages: Vec<GraphCommunityMarkdownPage>,
}

impl GraphCommunityMarkdownPlan {
    /// Find a page by its normalized relative path.
    pub fn page(&self, path: &str) -> Option<&GraphCommunityMarkdownPage> {
    ...
    pub fn page_by_graph_ref(
        &self,
        kind: &str,
        graph_ref: &str,
    ) -> Option<&GraphCommunityMarkdownPage> {
...
/// Render pure structural graph-community Markdown pages for graph annotations only.
pub fn render_graph_community_markdown(
    graph: &KnowledgeGraph,
    topics: &crate::graph_community_topics::GraphCommunityTopicTree,
) -> anyhow::Result<GraphCommunityMarkdownPlan> {
    render_graph_community_markdown_with_catalog(graph, topics, &BTreeMap::new())
}

/// Render structural graph-community Markdown pages, including every active Catalog annotation.
///
...
pub fn render_graph_community_markdown_with_catalog(
    graph: &KnowledgeGraph,
    topics: &crate::graph_community_topics::GraphCommunityTopicTree,
    ...
    pages.push(GraphCommunityMarkdownPage {
        path: "index.md".into(),
        markdown: root_page(
            &sorted_topics,
            &topic_paths,
    ...
    Ok(GraphCommunityMarkdownPlan { pages })
}
...
fn readable_topic_label(
    topic: &crate::graph_community_topics::GraphCommunityTopic,
...
fn is_topic_placeholder(topic: &crate::graph_community_topics::GraphCommunityTopic) -> bool {
...
fn root_page(
    topics: &[crate::graph_community_topics::GraphCommunityTopic],
...
fn source_page(
    ...
    community_paths: &BTreeMap<i64, String>,
    labels: &BTreeMap<i64, String>,
    topic_communities: &BTreeMap<i64, &crate::graph_community_topics::GraphCommunityTopic>,
    topic_paths: &BTreeMap<String, String>,
...
mod tests {
    ...
    fn fallback_navigation_titles_describe_a_multi_source_collection() {
        ...
        let topic = crate::graph_community_topics::GraphCommunityTopic {
            id: "topic-0".into(),
            label: "Topic 0".into(),
            communities: vec![1],
        };
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct GraphCommunityGodNodeArticle {
    pub id: String,
    pub label: String,
    pub degree: usize,
}
...
pub struct GraphCommunityMarkdownOptions {
    pub community_labels: BTreeMap<i64, String>,
    pub cohesion: BTreeMap<i64, f64>,
    pub god_nodes: Vec<GraphCommunityGodNodeArticle>,
}

#[derive(Debug, Clone, Default, PartialEq, Eq)]
pub struct GraphCommunityMarkdownReport {
...
pub fn export_graph_community_markdown(
    graph: &KnowledgeGraph,
    directory: &Path,
) -> anyhow::Result<()> {
    ...
    export_graph_community_markdown_with_options(graph, &communities, directory, &options)
        .map(|_| ())
}

pub fn export_graph_community_markdown_with_options(
    graph: &KnowledgeGraph,
    communities: &Communities,
    directory: &Path,
    options: &GraphCommunityMarkdownOptions,
) -> anyhow::Result<GraphCommunityMarkdownReport> {
    ...
    let by_id = graph
        ...
        .collect::<HashMap<_, _>>();
    let mut report = GraphCommunityMarkdownReport::default();
    ...
    if !communities.is_empty() && valid.is_empty() {
        anyhow::bail!("all community node IDs are stale; rebuild communities before graph-community Markdown export");
    }
    if report.stale_nodes_dropped > 0 {
        let warning = format!(
            "dropped {} stale community node ID(s) during graph-community Markdown export",
...
fn valid_god_data<'a>(
    options: &'a GraphCommunityMarkdownOptions,
    by_id: &HashMap<&str, &Node>,
) -> Vec<&'a GraphCommunityGodNodeArticle> {
    options
        .god_nodes
        .iter()
        .filter(|god| by_id.contains_key(god.id.as_str()))
...
fn index_markdown(
    ...
    gods: &[&GraphCommunityGodNodeArticle],
...
fn god_node_article(
    graph: &KnowledgeGraph,
    god: &GraphCommunityGodNodeArticle,
    node_community: &HashMap<&str, i64>,
    community_articles: &BTreeMap<i64, String>,
    god_articles: &BTreeMap<String, String>,
    options: &GraphCommunityMarkdownOptions,
...
fn community_label(community: i64, options: &GraphCommunityMarkdownOptions) -> String {
    options
        .community_labels
        .get(&community)
        .cloned()
