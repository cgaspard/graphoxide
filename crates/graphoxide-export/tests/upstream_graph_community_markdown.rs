...
use graphoxide_export::{
    derive_graph_community_topic_tree, export_graph_community_markdown_with_options,
    render_graph_community_markdown, render_graph_community_markdown_with_catalog, Communities,
    GraphCommunityGodNodeArticle, GraphCommunityMarkdownOptions, GraphCommunityTopic,
    GraphCommunityTopicTree,
};
use std::{
...
fn options() -> GraphCommunityMarkdownOptions {
    GraphCommunityMarkdownOptions {
        community_labels: BTreeMap::from([
            (0, "Parsing Layer".into()),
            (1, "Rendering Layer".into()),
        ]),
        cohesion: BTreeMap::from([(0, 0.85), (1, 0.72)]),
        god_nodes: vec![GraphCommunityGodNodeArticle {
            id: "n1".into(),
            label: "parse".into(),
            degree: 2,
        }],
...
fn export(
    ...
    options: &GraphCommunityMarkdownOptions,
) -> (TempDir, graphoxide_export::GraphCommunityMarkdownReport) {
    let temp = tempfile::tempdir().unwrap();
    let report =
        export_graph_community_markdown_with_options(graph, communities, temp.path(), options)
            .unwrap();
    (temp, report)
}
...
#[test]
fn test_to_graph_community_markdown_writes_index() {
    let (temp, _) = export(&graph(), &communities(), &options());
    assert!(temp.path().join("index.md").is_file());
}

#[test]
fn test_to_graph_community_markdown_returns_article_count() {
    let (_, report) = export(&graph(), &communities(), &options());
    assert_eq!(report.article_count, 3);
}

#[test]
fn test_to_graph_community_markdown_community_articles_created() {
    let (temp, _) = export(&graph(), &communities(), &options());
...
#[test]
fn test_to_graph_community_markdown_god_node_article_created() {
    let (temp, _) = export(&graph(), &communities(), &options());
...
#[test]
fn test_to_graph_community_markdown_skips_missing_god_node_ids() {
    let mut options = options();
    options.god_nodes = vec![GraphCommunityGodNodeArticle {
        id: "nonexistent".into(),
        label: "ghost".into(),
        degree: 99,
    }];
...
#[test]
fn test_to_graph_community_markdown_no_labels_uses_fallback() {
    let (temp, _) = export(
        &graph(),
        &communities(),
        &GraphCommunityMarkdownOptions::default(),
    );
...
fn test_community_article_truncation_notice() {
    ...
    let options = GraphCommunityMarkdownOptions {
...
#[test]
fn test_to_graph_community_markdown_drops_stale_community_nodes() {
...
#[test]
fn test_to_graph_community_markdown_all_stale_raises() {
    let temp = tempfile::tempdir().unwrap();
    ...
    let error =
        export_graph_community_markdown_with_options(&graph(), &stale, temp.path(), &options())
            .unwrap_err();
    assert!(error.to_string().contains("stale"));
}

#[test]
fn test_to_graph_community_markdown_stale_nodes_prints_warning() {
    let communities = BTreeMap::from([
...
#[test]
fn test_to_graph_community_markdown_case_only_distinct_labels_dont_overwrite() {
...
#[test]
fn test_to_graph_community_markdown_god_node_label_case_collides_with_community() {
    let graph = KnowledgeGraph {
        nodes: vec![
...
#[test]
fn test_graph_community_markdown_emits_no_obsidian_wikilinks() {
    let (temp, _) = export(&graph(), &communities(), &options());
...
#[test]
fn test_graph_community_markdown_links_resolve_to_real_files() {
    let (temp, _) = export(&graph(), &communities(), &options());
    let mut seen = false;
    for entry in fs::read_dir(temp.path()).unwrap().filter_map(Result::ok) {
        if entry.path().extension().and_then(|value| value.to_str()) != Some("md") {
...
#[test]
fn test_graph_community_markdown_link_display_keeps_label_but_target_is_filename() {
    let (temp, _) = export(&graph(), &communities(), &options());
    let index = read(temp.path(), "index.md");
    assert!(index.contains("[Parsing Layer](Parsing_Layer.md)"));
...
#[test]
fn test_graph_community_markdown_special_characters_in_label_resolve() {
    let (graph, communities) = collision_graph();
    let options = GraphCommunityMarkdownOptions {
...
#[test]
fn test_graph_community_markdown_link_with_bracketed_label_resolves() {
    let (graph, communities) = collision_graph();
    let options = GraphCommunityMarkdownOptions {
...
#[test]
fn test_graph_community_markdown_links_to_nodes_without_articles_are_plain_text() {
    let (temp, _) = export(&graph(), &communities(), &options());
    let article = read(temp.path(), "parse.md");
    assert!(article.contains("- validate") && article.contains("- render"));
    assert!(!article.contains("[[validate]]") && !article.contains("[[render]]"));
...
#[test]
fn test_graph_community_markdown_links_use_collision_suffixed_slug() {
    let (graph, communities) = collision_graph();
    let options = GraphCommunityMarkdownOptions {
        community_labels: BTreeMap::from([(0, "Parser".into()), (1, "parser".into())]),
        ..GraphCommunityMarkdownOptions::default()
...
fn structured_tree() -> GraphCommunityTopicTree {
    GraphCommunityTopicTree {
        topics: vec![GraphCommunityTopic {
            id: "topic-0".into(),
            label: "Language".into(),
            communities: vec![1, 2],
        }],
...
fn chunking_fixture(count: usize) -> (KnowledgeGraph, GraphCommunityTopicTree) {
    let graph = KnowledgeGraph {
        nodes: (0..count)
            .rev()
            .map(|index| {
...
#[test]
fn structured_graph_community_markdown_materializes_every_active_catalog_source_and_unrepresented_inventory(
) {
    let graph = KnowledgeGraph {
    ...
    let tree = GraphCommunityTopicTree {
        topics: vec![GraphCommunityTopic {
            id: "topic-0".into(),
            label: "GraphCommunityTopic".into(),
            communities: vec![1],
        }],
    ...
    let plan = render_graph_community_markdown_with_catalog(&graph, &tree, &annotations).unwrap();
...
#[test]
fn structured_graph_community_markdown_graph_only_wrapper_retains_the_catalog_free_render_plan() {
    let graph = structured_graph();
    assert_eq!(
        render_graph_community_markdown(&graph, &structured_tree()).unwrap(),
        render_graph_community_markdown_with_catalog(&graph, &structured_tree(), &BTreeMap::new())
            .unwrap()
    );
}

#[test]
fn structured_graph_community_markdown_rejects_catalog_graph_active_capture_mismatches() {
    let graph = KnowledgeGraph {
        nodes: vec![catalog_node(
            "extracted-node",
            "Extracted",
    ...
    assert!(render_graph_community_markdown_with_catalog(&graph, &tree, &annotations).is_err());
}

#[test]
fn structured_graph_community_markdown_rejects_catalog_annotations_bound_to_another_source_path() {
    let graph = KnowledgeGraph {
        nodes: vec![catalog_node(
            "misbound-node",
            "Misbound",
    ...
    assert!(
        render_graph_community_markdown_with_catalog(&graph, &structured_tree(), &annotations)
            .is_err()
    );
}

#[test]
fn structured_graph_community_markdown_rejects_an_unannotated_node_from_an_active_catalog_source() {
    let graph = KnowledgeGraph {
...
#[test]
fn structured_graph_community_markdown_binds_container_members_to_their_active_container_source() {
...
#[test]
fn structured_graph_community_markdown_covers_each_annotated_source_once_and_links_community_sources(
) {
    let plan = render_graph_community_markdown(&structured_graph(), &structured_tree()).unwrap();
    let paths = plan
        .pages
        .iter()
    ...
    assert!(plan.page("topics/topic-0.md").is_none());
    assert!(plan.page_by_graph_ref("topic", "topic-0").is_some());
    assert!(plan.page_by_graph_ref("community", "1").is_some());
    assert!(plan.page_by_graph_ref("community", "2").is_some());
    ...
    let parser_community = plan.page_by_graph_ref("community", "1").unwrap();
...
#[test]
fn structured_graph_community_markdown_assigns_shared_source_one_primary_community() {
    ...
    let plan = render_graph_community_markdown(&graph, &structured_tree()).unwrap();
    let source = plan.page("sources/shared.md").unwrap();
    let primary = plan
        .page_by_graph_ref("community", "1")
        .unwrap()
        .path
        .as_str();
    let secondary = plan
        .page_by_graph_ref("community", "2")
        .unwrap()
        .path
        .as_str();
...
#[test]
fn structured_graph_community_markdown_is_identical_after_shuffling_graph_input() {
    let graph = structured_graph();
    let expected = render_graph_community_markdown(&graph, &structured_tree()).unwrap();
    let mut shuffled = graph;
    shuffled.nodes.reverse();
    shuffled.links.reverse();
    let actual = render_graph_community_markdown(&shuffled, &structured_tree()).unwrap();
    assert_eq!(actual, expected);
}

#[test]
fn structured_graph_community_markdown_orders_root_links_by_label_and_stable_topic_id() {
    let graph = KnowledgeGraph {
        nodes: vec![
    ...
    let plan = render_graph_community_markdown(&graph, &tree).unwrap();
    let index = &plan.page("index.md").unwrap().markdown;
    assert_eq!(index.matches("### Topics 1–3").count(), 1);
    let first_path = &plan.page_by_graph_ref("topic", "topic-1").unwrap().path;
    let second_path = &plan.page_by_graph_ref("topic", "topic-2").unwrap().path;
    let third_path = &plan.page_by_graph_ref("topic", "topic-0").unwrap().path;
    ...
    assert_eq!(
        ...
        render_graph_community_markdown(&graph, &relabeled)
            .unwrap()
            .pages
            .iter()
            .map(|page| &page.path)
...
#[test]
fn structured_graph_community_markdown_propagates_explicit_fallbacks_without_path_or_citation_changes(
) {
    ...
    let baseline = render_graph_community_markdown(
        &semantic,
        &derive_graph_community_topic_tree(&semantic).unwrap(),
    )
    .unwrap();

    let plan = render_graph_community_markdown(
        &graph,
        &derive_graph_community_topic_tree(&graph).unwrap(),
    )
    .unwrap();
    assert_eq!(
        plan.pages.iter().map(|page| &page.path).collect::<Vec<_>>(),
...
#[test]
fn structured_graph_community_markdown_chunks_the_root_without_adding_pages() {
    ...
    for topic in 0..101 {
        assert!(plan
            .page_by_graph_ref("topic", &format!("topic-{topic}"))
            .is_some());
    }
    assert!(!plan
        .pages
...
#[test]
fn structured_graph_community_markdown_root_chunking_is_deterministic_after_shuffling() {
    let (graph, tree) = chunking_fixture(101);
    let expected = render_graph_community_markdown(&graph, &tree).unwrap();
    ...
    assert_eq!(
        render_graph_community_markdown(&shuffled_graph, &shuffled_tree).unwrap(),
        expected
    );
}

#[test]
fn structured_graph_community_markdown_keeps_normal_sized_community_citations_and_links_every_source_page(
) {
    let graph = KnowledgeGraph {
        nodes: (0..13)
            .map(|index| {
    ...
    let page = render_graph_community_markdown(&graph, &tree)
        .unwrap()
        .page_by_graph_ref("community", "1")
        .unwrap()
        .markdown
...
fn page_digest(page: &graphoxide_export::GraphCommunityMarkdownPage) -> String {
    page.markdown
        .lines()
...
fn serialized_frontmatter_bytes(page: &graphoxide_export::GraphCommunityMarkdownPage) -> usize {
    page.markdown.find("\n---\n\n").unwrap() + "\n---\n\n".len()
}

#[test]
fn structured_graph_community_markdown_bounds_catalog_id_filenames_and_serialized_frontmatter() {
    ...
    let community = plan.page_by_graph_ref("community", "1").unwrap();
    ...
    assert!(render_graph_community_markdown(&invalid, &structured_tree()).is_err());
}

#[test]
fn structured_graph_community_markdown_allows_an_exact_full_frontmatter() {
    let tree = GraphCommunityTopicTree {
        topics: vec![GraphCommunityTopic {
            id: "topic-0".into(),
            label: "GraphCommunityTopic".into(),
    ...
    let base = serialized_frontmatter_bytes(
        render_graph_community_markdown(&probe, &tree)
            .unwrap()
            .page_by_graph_ref("community", "1")
            .unwrap(),
    ) - "  - p#c\n".len();
    ...
    let page = render_graph_community_markdown(&graph_for(final_citation), &tree)
        .unwrap()
        .page_by_graph_ref("community", "1")
        .unwrap()
        .clone();
    ...
    let overfull = render_graph_community_markdown(&graph_for(final_citation + 1), &tree)
        .unwrap()
        .page_by_graph_ref("community", "1")
        .unwrap()
        .clone();
...
#[test]
fn structured_graph_community_markdown_digests_track_capture_hashes_and_placement() {
    let graph = structured_graph();
    let baseline = render_graph_community_markdown(&graph, &structured_tree()).unwrap();
    ...
    let hashed = render_graph_community_markdown(&changed_hash, &structured_tree()).unwrap();
    for (baseline_page, hashed_page) in [
        (
            baseline.page("index.md").unwrap(),
            hashed.page("index.md").unwrap(),
        ),
        (
            baseline.page_by_graph_ref("topic", "topic-0").unwrap(),
            hashed.page_by_graph_ref("topic", "topic-0").unwrap(),
        ),
        (
            baseline.page_by_graph_ref("community", "1").unwrap(),
            hashed.page_by_graph_ref("community", "1").unwrap(),
        ),
        (
            baseline.page("sources/parser.md").unwrap(),
            hashed.page("sources/parser.md").unwrap(),
        ),
    ] {
        assert_ne!(page_digest(baseline_page), page_digest(hashed_page));
    }
    for (path, baseline_page, hashed_page) in [
        (
            "community 2",
            baseline.page_by_graph_ref("community", "2").unwrap(),
            hashed.page_by_graph_ref("community", "2").unwrap(),
        ),
        (
            "sources/client.md",
            baseline.page("sources/client.md").unwrap(),
            hashed.page("sources/client.md").unwrap(),
        ),
        (
            "sources/shared.md",
            baseline.page("sources/shared.md").unwrap(),
            hashed.page("sources/shared.md").unwrap(),
        ),
        (
            "sources/loose.md",
            baseline.page("sources/loose.md").unwrap(),
            hashed.page("sources/loose.md").unwrap(),
        ),
    ] {
        assert_eq!(
            page_digest(baseline_page),
            page_digest(hashed_page),
    ...
    let placed = render_graph_community_markdown(&moved, &structured_tree()).unwrap();
    assert_ne!(
        page_digest(baseline.page("sources/shared.md").unwrap()),
        page_digest(placed.page("sources/shared.md").unwrap())
    );
    assert_ne!(
        page_digest(baseline.page_by_graph_ref("community", "1").unwrap()),
        page_digest(placed.page_by_graph_ref("community", "1").unwrap())
    );
    assert_eq!(
    ...
    let split_tree = GraphCommunityTopicTree {
        topics: vec![
            GraphCommunityTopic {
                id: "topic-0".into(),
                label: "Language".into(),
    ...
    let repartitioned = render_graph_community_markdown(&structured_graph(), &split_tree).unwrap();
    for (baseline_page, repartitioned_page) in [
        (
            baseline.page("index.md").unwrap(),
            repartitioned.page("index.md").unwrap(),
        ),
        (
            baseline.page_by_graph_ref("topic", "topic-0").unwrap(),
            repartitioned.page_by_graph_ref("topic", "topic-0").unwrap(),
        ),
        (
            baseline.page_by_graph_ref("community", "2").unwrap(),
            repartitioned.page_by_graph_ref("community", "2").unwrap(),
        ),
        (
            baseline.page("sources/shared.md").unwrap(),
            repartitioned.page("sources/shared.md").unwrap(),
        ),
    ] {
        assert_ne!(page_digest(baseline_page), page_digest(repartitioned_page));
    }
}

#[test]
fn structured_graph_community_markdown_escapes_backslashes_before_brackets_in_links() {
    let mut graph = structured_graph();
    ...
    let plan = render_graph_community_markdown(&graph, &tree).unwrap();
    let topic_path = &plan.page_by_graph_ref("topic", "topic-0").unwrap().path;
    let community_path = &plan.page_by_graph_ref("community", "1").unwrap().path;
    assert!(plan
        .page("index.md")
        .unwrap()
        .markdown
    ...
    assert!(plan
        .page_by_graph_ref("topic", "topic-0")
        .unwrap()
        .markdown
...
#[test]
fn structured_graph_community_markdown_recomputes_semantic_community_name_after_shuffling() {
    let mut graph = structured_graph();
    ...
    graph.nodes.reverse();
    graph.links.reverse();
    let actual = render_graph_community_markdown(&graph, &structured_tree()).unwrap();
    assert_eq!(actual, expected);
    assert!(expected
        .page_by_graph_ref("community", "1")
        .unwrap()
...
#[test]
fn structured_graph_community_markdown_renders_related_communities_only_from_graph_links() {
    let mut graph = KnowledgeGraph {
        nodes: vec![
    ...
    let expected = render_graph_community_markdown(&graph, &tree).unwrap();
    let first = expected.page_by_graph_ref("community", "1").unwrap();
    let second_path = &expected.page_by_graph_ref("community", "2").unwrap().path;
    ...
    let reweighted = render_graph_community_markdown(&reweighted_graph, &tree).unwrap();
    assert_ne!(
        page_digest(first),
        page_digest(reweighted.page_by_graph_ref("community", "1").unwrap())
    );

    graph.nodes.reverse();
    graph.links.reverse();
    assert_eq!(
        render_graph_community_markdown(&graph, &tree).unwrap(),
        expected
    );
}

#[test]
fn structured_graph_community_markdown_keeps_repeated_v1_sources_distinct_and_renders_provenance() {
    let graph = KnowledgeGraph {
        nodes: vec![
            catalog_node(
                "shared-one",
    ...
    let changed =
        render_graph_community_markdown_with_catalog(&graph, &tree, &changed_annotations).unwrap();
    assert_ne!(
        page_digest(source),
    ...
    let inventory_changed =
        render_graph_community_markdown_with_catalog(&graph, &tree, &inventory_annotations)
            .unwrap();
    assert_ne!(
        page_digest(inventory),
        page_digest(
...
#[test]
fn structured_graph_community_markdown_tolerates_partial_graph_only_provenance() {
    ...
    assert!(
        render_graph_community_markdown_with_catalog(&graph, &tree, &active_annotations).is_err()
    );
}

#[test]
fn structured_graph_community_markdown_merges_partial_graph_only_provenance_independently_of_node_order(
) {
...
#[test]
fn structured_graph_community_markdown_materializes_each_unrepresented_repeated_v1_capture() {
    ...
    let tree = GraphCommunityTopicTree::default();

    let expected =
        render_graph_community_markdown_with_catalog(&graph, &tree, &annotations).unwrap();
    for (capture, path) in [
    ...
    assert_eq!(
        render_graph_community_markdown_with_catalog(&graph, &tree, &reversed).unwrap(),
        expected
    );
}
