...
use graphoxide_export::{derive_graph_community_topic_tree, GraphCommunityTopicTree};
...
fn assert_complete(tree: &GraphCommunityTopicTree, communities: &[i64]) {
    let mut actual = tree
        .topics
        .iter()
...
fn shuffled_graph_input_has_the_same_taxonomy() {
    ...
    let expected = derive_graph_community_topic_tree(&graph).unwrap();

    let mut shuffled = graph;
    shuffled.nodes.reverse();
    shuffled.links.reverse();
    let actual = derive_graph_community_topic_tree(&shuffled).unwrap();
...
fn disconnected_community_is_retained_in_the_taxonomy() {
    let tree = derive_graph_community_topic_tree(&graph(
...
fn topic_uses_the_most_cross_community_connected_child_label() {
    ...
    let tree = derive_graph_community_topic_tree(&graph).unwrap();
    let topic = tree
        .topics
        .iter()
